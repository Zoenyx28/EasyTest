...
async function handleSubmit() {
  ...
  try {
    // 更新指派对象
    await defectApi.updateDefect(props.defect.id, { assignee_id: assigneeId.value });
    // 备注非空时写入评论
    if (comment.value.trim()) {
      await defectApi.createComment(props.defect.id, comment.value.trim());
    }
    emit('done');
    emit('showToast', '指派更新成功');
    emit('close');
  } catch (e: any) {
    emit('showToast', e.message || '指派失败');
  } finally {
    saving.value = false;
  }
}
    ...
    <div v-if="isOpen" class="dialog-overlay" @click.self="emit('close')">
      <div class="dialog-container">
        <div class="dialog-header">
          <h2 class="dialog-title">重新指派 #{{ defect?.id }}</h2>
